...
public class BotUltime extends Bot {
	...
	/**
	 * L'Ordre d'importance 1 . Dé 1 valeur Or > VALEUR_OR_MINIMUM_DE1 2 . Dé 1
	 * valeur Lune > VALEUR_LUNE_MINIMUM_DE1 3 . Dé 2 valeur Soleil >
	 * VALEUR_SOLEIL_MINIMUM_DE2 4 . Dé 2 valeur Lune > 1 5 . Dé 1 Echange (Face
	 * Or_1 par Face Victoire_3 & 4) ou (Si plus de Marteau échange Face Or min par
	 * Face Victoire dispo Max) 6 . Dé 2 Echange Face Or-1 par Face
	 * OR_2_OU_LUNE_2_OU_SOLEIL_2 7 . A voir
	 */
	...
	private static final int VALEUR_LUNE_MINIMUM_DE2 = 2;
	private static final int VALEUR_SOLEIL_MINIMUM_DE2 = 2;
	private static final int PRIX_CASQUE_CARTE = 5;

	/**
	 * Donne le nombres de ressources en plus de l'Or que l'on peut avoir en même
	 * temps sur cette face
	 */
	...
	/**
	 * Donne le nombres de ressources en plus de la Lune que l'on peut avoir en même
	 * temps sur cette face
	 */
	...
	/**
	 * Donne le nombres de ressources en plus des Pdv que l'on peut avoir en même
	 * temps sur cette face
	 */
	...
	/**
	 * Donne le nombres de ressources en plus des Soleils que l'on peut avoir en
	 * même temps sur cette face
	 */
	...
	/**
	 * donne l'indice de la Face qui va être appliquée de manière négatif sur le
	 * joueur
	 */
	@Override
	public int choixFaceNegatif(Face... faces) {
		...
		for (int indice = 0; indice < faces.length; indice++) {
			...
			// On choisit alors la face qui nous fait perdre le moins d'Or et
			// devaleurAccesoire
	...
	private int getMinIndiceVictoire(De de) {
		...
		for (int indice = 0; indice < 6; indice++) {
			...
			if (valueVictoire < poidFace && valueVictoire >= 1
					&& de.getFace(indice) != Faces.OR_1_LUNE_1_SOLEIL_1_VICTOIRE_1) {
				choix = indice;
	...
	private int getMinIndiceSoleil(De de) {
		...
		for (int indice = 0; indice < 6; indice++) {
			...
			if (valueSoleil < poidFace && valueSoleil >= 1
					&& de.getFace(indice) != Faces.OR_1_LUNE_1_SOLEIL_1_VICTOIRE_1) {
				choix = indice;
	...
	public void forge(Face face) {
		...
		// On traite MIROIR
		// Normalement impossible que notre joueur en achète, mais on traite ce cas
		// quand même.
		// Si il y en a un, il est placé à la place de la face rapportant le moins de
		// victoire.
		...
		if (face == Faces.OR_1 || face == Faces.OR_2 || face == Faces.OR_3 || face == Faces.OR_4 || face == Faces.OR_6
				|| face == Faces.OR_3_OU_VICTOIRE_2) {
		...
		if (face == Faces.VICTOIRE_1 || face == Faces.VICTOIRE_2 || face == Faces.VICTOIRE_3
				|| face == Faces.VICTOIRE_4) {
			getJoueur().de1.forge(face, minIndiceOrDe1);
			return;
		}
	...
	public void faireAchatsFace(ListeAchat liste) {
		...
		while (!liste.isEmpty()) {
			...
			// Si il n'a pas fait d'action on s'arrête, car cela veut dire qu'il n'y a plus
			// rien d'intéressant pour le joueur à acheter
			if (!action) {
				break;
			}
	...
	public boolean tourSanctuaire() {
		...
		// Dé 1 valeur Or < VALEUR_OR_MINIMUM_DE1 && qu'on peut avoir une Face >= OR_3
		// on va la prendre
		if (BotUtils.valeurDe(VALEUR_OR, getJoueur().de1) < VALEUR_OR_MINIMUM_DE1
				&& ((listAchetable.contains(Faces.OR_3) && this.getJoueur().getOr() >= 2)
						|| (listAchetable.contains(Faces.OR_3_OU_VICTOIRE_2) && getJoueur().getOr() >= 5)
						|| (listAchetable.contains(Faces.OR_4) && getJoueur().getOr() >= 3)
						|| (listAchetable.contains(Faces.OR_6) && getJoueur().getOr() >= 4))) {
			return true;
		}
		// Marteau du Forgeron : en avoir toujours 1 si il en reste.
		if (this.getJoueur().getNextMarteau() == null
				&& nombreCartes(this.getJoueur().getPartie().getPlateau().getCartes(), Cartes.MARTEAU) > 0) {
			return false;
		}
		// Si on n'a toujours pas augmenté notre stock max il faut le faire car on
		// risque d'être limité donc
		// si on peut acheter le Coffre du forgeron et qu'on n'en a pas encore, on le
		// prend.
		...
		// Dé 2 valeur Soleil <= 2 && qu'on peut avoir une Face >= Soleil 1 on va la
		// prendre
		if (BotUtils.valeurDe(VALEUR_SOLEIL, getJoueur().de2) <= VALEUR_SOLEIL_MINIMUM_DE2
				&& listAchetable.contains(Faces.SOLEIL_1) && this.getJoueur().getOr() >= 2)
			return true;

		// Dé 2 valeur Lune < 1 && qu'on peut avoir une Face >= Lune_1 on va la prendre
		if (BotUtils.valeurDe(VALEUR_LUNE, getJoueur().de2) < VALEUR_LUNE_MINIMUM_DE2
				&& listAchetable.contains(Faces.LUNE_1) && this.getJoueur().getOr() >= 2)
			return true;

		// Si on a un marteau en cours on va voir si on peut acheter une carte
		// intéressante
		if (this.getJoueur().getNextMarteau() != null)
			return false;
	...
	public boolean faireTourSupplementaire() {
		...
		// 1 Coffre Forgeron
		if (this.getJoueur().getOrMax() == VALEUR_MAX_RESERVE_OR_INIT && this.getJoueur().getLune() >= 1
				&& nombreCartes(getJoueur().getPartie().getPlateau().getCartes(), Cartes.COFFRE) > 0) {
			return true;
		}
	...
	public Carte faireAchatCartes(List<Carte> cartes) {
		...
		// Si il n'y a plus de marteau achetable il commence à faire une transition avec
		// la carte Ancien
		if (nombreCartes(cartes, Cartes.ANCIEN) > 0) {
	...
	public CarteRenfort choixRenfort(List<CarteRenfort> liste) {
		...
		if (liste.contains(Cartes.SABOTS)) {
			return Cartes.SABOTS;
		} else if (liste.contains(Cartes.AILES)) {
			return Cartes.AILES;
		} else if (liste.contains(Cartes.ANCIEN) && Cartes.ANCIEN.peutActiver(getJoueur())) {
			return Cartes.ANCIEN;
		}
		return null;
	}
	...
	public int changeOrEnMarteau(int orChangeable) {
		// Si il a le minimum de valeur par Dé
		if (BotUtils.valeurDe(VALEUR_OR, getJoueur().de1) <= VALEUR_OR_MINIMUM_DE1
				&& BotUtils.valeurDe(VALEUR_LUNE, getJoueur().de1) < VALEUR_LUNE_MINIMUM_DE1
				&& BotUtils.valeurDe(VALEUR_SOLEIL, getJoueur().de2) < VALEUR_SOLEIL_MINIMUM_DE2
				&& BotUtils.valeurDe(VALEUR_LUNE, getJoueur().de2) < VALEUR_LUNE_MINIMUM_DE2) {
			return orChangeable/3;
		}
		return orChangeable;
	}
}